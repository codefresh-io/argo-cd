...
import (
	"context"
	"fmt"
	"github.com/argoproj/argo-cd/v2/application_change_revision_controller"
...
func NewCommand() *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			changeRevisionServerOpts := application_change_revision_controller.ApplicationChangeRevisionServerOpts{
			...
			log.Infof("Starting change revision controller server with grpc transport %v", useGrpc)

			stats.RegisterStackDumper()
			stats.StartStatsTicker(10 * time.Minute)
			stats.RegisterHeapDumper("memprofile")
			changeRevisionServer := application_change_revision_controller.NewApplicationChangeRevisionServer(ctx, changeRevisionServerOpts)
	...
	command.Flags().IntVar(&metricsPort, "metrics-port", common.DefaultPortCRCServerMetrics, "Start metrics on given port")
